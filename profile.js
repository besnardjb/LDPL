...
function fill_prof_table()
{
	...
	for( var i = 0 ; i < profile_dat.length ; i++ )
	{
		...
		tcontent += "<thead><tr><th>Function</th><th class='sorttable_numeric'  >Calls</th><th class='sorttable_numeric'  >Total Time</th><th class='sorttable_numeric'  >Average Time</th> <th class='sorttable_numeric' >Min Time</th><th class='sorttable_numeric'  >Max Time</th></tr></thead>";
		...
		for( var k = 0 ; k < eprof.length ; k++ )
		{
			...
			tcontent += "<tr><td>" +e.name + "</td><td>" + e.call + "</td><td>" + to_time( e.total_time ) + "</td> <td>" + to_time( parseFloat(e.total_time) / parseFloat(e.call) ) + "</td> <td>" + to_time( e.min_time ) + "</td><td>" + to_time(e.max_time) + "</td></tr>";
		}
